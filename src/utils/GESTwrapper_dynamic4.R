GESTwrapper.dynamic4 <- function(configuration, locations, binSize) {
    ...
    if (!identical(finishedLocationsPrevious,finishedLocationsAfter) && stopCriterionReached == 0) {
      if (length(examineSubset)!=nVisibleNodes) {
        ...
        if (length(futureLocations) != 1) {
          ...
          if (max(combinedValue[futureLocations]) < 8) {
...
#     }
  ...
  thresholdEstimations <- sapply(states, function(x) x$domain[which.min(abs(cumsum(x$pdf) - 0.5))])
  ...
  return(list(thresholdDeviations, nrOfSteps, totDev,totThresh,thresholdEstimations))
}
