crfVisualField106_G <- function(normThresholds, normThresholdSDs, edgeSD, clampV,maxIter) {
...
#   edges <- list(c(2),c(3,1),c(4,2),c(3),c(),c(1,7,13,5),c(2,8,14,6),c(3,9,15,7),c(4,10,16,8),c(),
#                 c(),c(5,13,21,11),c(6,14,22,12),c(7,15,23,13),c(8,16,24,14),c(9,17,25,15),c(10,18,26,16),
#                 c(),c(),c(11,21,31,19),c(12,22,32,20),c(13,23,33,21),c(14,24,34,22),c(15,25,35,23),
#                 c(16,26,36,24),c(17,27,37,25),c(18,28,38,26),c(),c(),c(19,31,43,29),c(20,32,44,30),
#                 c(21,33,45,31),c(22,34,46,32),c(23,35,47,33),c(24,36,48,34),c(25,37,49,35),c(26,38,36),c(27,39,51,37),
#                 c(28,40,52,38),c(53),c(54),c(29,43,55),c(30,44,56,42),c(31,45,57,43),c(32,46,58,44),
#                 c(33,47,59,45),c(34,48,60,46),c(35,49,61,47),c(36,62,48),c(),c(38,52,64),c(39,53,65,51),c(40,66),
#                 c(),c(42,56,67,54),c(43,57,68,55),c(44,58,69,56),c(45,59,70,57),c(46,60,71,58),c(47,61,72,59),
#                 c(48,62,73,60),c(49,74,61),c(),c(51,65,76),c(52,66,77,64),c(53,78),
#                 c(),c(56,69,79,67),c(57,70,80,68),c(58,71,81,69),c(59,72,82,70),c(60,73,83,71),c(61,74,84,72),
#                 c(62,75,85,73),c(76,86,74),c(64,77,87,75),c(65,78,88,76),c(66),c(),c(69,81,89,79),c(70,82,90,80),
#                 c(71,83,91,81),c(72,84,92,82),c(73,85,93,83),c(74,86,94,84),c(75,87,95,85),c(76,88,96,86),c(),
#                 c(),c(81,91,97,89),c(82,92,98,90),c(83,93,99,91),c(84,94,100,92),c(85,95,101,93),c(86,96,102,94),c(),
#                 c(),c(91,99,103,97),c(92,100,104,98),c(93,101,105,99),c(94,102,106,100),c(),
#                 c(104),c(105,103),c(106,104),c(105)
#   )
...
#   decResWthClmpd[clampedLocations] <- clampV[clampedLocations]
  ...
  for (i in 1:nVisibleNodes) {
    diffsToRight[i] <- infResWthClmpd[[rightNeighbours[[visibleNodes[[i]]]]]] - infResWthClmpd[[visibleNodes[[i]]]]
    diffsToLeft[i] <- infResWthClmpd[[leftNeighbours[[visibleNodes[[i]]]]]] - infResWthClmpd[[visibleNodes[[i]]]]
    diffsToUp[i] <- infResWthClmpd[[upperNeighbours[[visibleNodes[[i]]]]]] - infResWthClmpd[[visibleNodes[[i]]]]
    diffsToDown[i] <- infResWthClmpd[[lowerNeighbours[[visibleNodes[[i]]]]]] - infResWthClmpd[[visibleNodes[[i]]]]
  }
...
#   patDf <- data.frame(x=visibleNodeXCoordinates,y=visibleNodeYCoordinates, patThresh = patientThresholds)
  ...
  gradsDf <- data.frame(x=visibleNodeXCoordinates, y=visibleNodeYCoordinates, grads = grads)
  combinedDf <- data.frame(x=visibleNodeXCoordinates, y=visibleNodeYCoordinates, values = grads+entropies[visibleNodes])
  # create plots
  ...
  title <- c("Gradients")
  plot3 <- ggplot(data = gradsDf, aes(x,y)) + 
    geom_tile(aes_string(fill = "grads"), colour = "white") + 
    scale_fill_gradient(low="white", high="blue" ,limits = c(0,2),name = "Gradients") + 
    labs(x="x [degrees]",y="y [degrees]",title=title) +
    geom_text(aes(label=paste(round(grads,digits=1))), size = sizeAnnotNumbers) +
    # annotate("text",x=-31,y=24,label=paste("Tot. dev. =",totDev,"\n("),hjust=0) +
    theme(axis.text = element_text(size=sizeAxisLables),plot.title = element_text(hjust = 0, vjust=0, size = sizeTitles),
          legend.position="none",axis.title=element_text(size=sizeAxisTitles),aspect.ratio = 1) 
  
  title <- c("Entropy + Gradients")
  plot4 <- ggplot(data = combinedDf, aes(x,y)) + 
    geom_tile(aes_string(fill = "values"), colour = "white") + 
    scale_fill_gradient(low="white", high="blue" ,limits = c(1,7),name = "Combined") + 
    labs(x="x [degrees]",y="y [degrees]",title=title) +
    geom_text(aes(label=paste(round(values,digits=1))), size = sizeAnnotNumbers) +
    # annotate("text",x=-31,y=24,label=paste("Tot. dev. =",totDev,"\n("),hjust=0) +
    theme(axis.text = element_text(size=sizeAxisLables),plot.title = element_text(hjust = 0, vjust=0, size = sizeTitles),
          legend.position="none",axis.title=element_text(size=sizeAxisTitles),aspect.ratio = 1) 
  
  plotRes <- grid.arrange(grobs=list(plot1,plot2,plot3,plot4),layout_matrix=matrix(c(1,2,3,4),ncol=4))
