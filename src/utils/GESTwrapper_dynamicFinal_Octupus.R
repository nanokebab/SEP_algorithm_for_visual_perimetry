GESTwrapper.dynamicFinal <- function(configuration, locations, binSize) {
  ...
  states <- lapply(locations, function(loc) {
    GEST.start(
      ...
      stopType = configuration[[4]], stopValue = configuration[[5]], 
  ...
  for (d in 1:nVisibleNodes) {
    ...
    nvDistrs[visibleNodes[[d]],] <- dnorm(supportCRF, normThresholds[[d]], priorSD, log = FALSE)
  }
...
#     }
  ...
  return(list(totThresh,nrOfSteps))
}
