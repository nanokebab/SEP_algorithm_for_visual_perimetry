dynamic.ANCHOR.step <- function (state, nextStim = NULL) 
{
  #load functions
  funPath <- "src/utils/"
  source(paste(funPath, "newStim.R",sep = ""))
  
  tableSteps <- c(6,8,8,8,8)
  ...
  if (opiResp$seen) {
    state$finished <- "Seen"
    if (state$numPresentations == 1) {
      state$stairResult <- state$startingEstimate
    } else if (state$numPresentations == 2) {
      state$stairResult <- newStim(tail(state$stimuli,1),tableSteps[[1]]/2)
    } else if (state$numPresentations > 2) {
      state$stairResult <- newStim(tail(state$stimuli,1),tableSteps[[2]]/2)
    }
    ...
    newStimulus <- newStim(tail(state$stimuli,1),-tableSteps[[state$numPresentations]])
...
environment(dynamic.ANCHOR.step) <- asNamespace('OPI')
