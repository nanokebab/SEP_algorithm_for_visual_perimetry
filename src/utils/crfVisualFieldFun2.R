crfVisualField <- function(normThresholds, normThresholdSDs, edgeSD, clampV, patientThresholds) {
  ...
  experimentNo <- 1
  data(vf91016left)
  patientThresholds <- as.vector(do.call(cbind,vf91016right[experimentNo,17:70]))
  ...
  edges <- list(c(2,6),c(3,7,1),c(4,8,2),c(9,3),c(6,12),c(1,7,13,5),c(2,8,14,6),c(3,9,15,7),c(4,10,16,8),c(17,9),
                c(12,20),c(5,13,21,11),c(6,14,22,12),c(7,15,23,13),c(8,16,24,14),c(9,17,25,15),c(10,18,16),
                c(27,17),c(20,28),c(11,21,29,19),c(12,22,30,20),c(13,23,31,21),c(14,24,32,22),c(15,25,33,23),
                c(16,34,24),c(),c(18,36),c(19,29),c(20,30,37,28),c(21,31,38,29),c(22,32,39,30),
  ...
  for (d in 1:nNodes) {
    nvDistrs[d,] <- dnorm(domain, normThresholds[[d]], normThresholdSDs[[d]], log = FALSE)
  }
...
#   patDf <- data.frame(x=nodeXCoordinates,y=nodeYCoordinates, patThresh = patientThresholds)
  ...
  totDev <- round(sum(abs(decResWthClmpd-patientThresholds)),1)
  priorDev <- round((sum(abs(decResWthClmpd-patientThresholds))) / (sum(abs(decPrior-patientThresholds))),2) * 100
