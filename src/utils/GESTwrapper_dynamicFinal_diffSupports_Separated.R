GESTwrapper.dynamicFinal <- function(configuration, locations, binSize=NULL) {
  ...
  supportCRF <- seq(-20,80,binSizeCRF)
  ...
  states <- lapply(locations, function(loc) {
    GEST.start(
      domain = supportStates, prior = dnorm(
  ...
  edges <- list(c(2,6),c(3,7,1),c(4,8,2),c(9,3),c(6,12),c(1,7,13,5),c(2,8,14,6),c(3,9,15,7),c(4,10,16,8),c(17,9),
                ...
                c(28,40,52,38),c(53,39),c(42),c(29,43,41),c(30,44,42),c(31,45,43),c(32,46,44),
                c(33,47,45),c(34,48,46),c(35,49,47),c(36,48),c(),c(38,52),c(39,53,51),c(40,52),
                c(55),c(56,67,54),c(57,68,55),c(58,69,56),c(59,70,57),c(60,71,58),c(61,72,59),
                c(62,73,60),c(74,61),c(),c(65,76),c(66,77,64),c(78,65),
  ...
  for (d in 1:nVisibleNodes) {
    nvDistrs[visibleNodes[[d]],] <- dnorm(supportCRF, locations[[d]][[4]], ifelse(d==26 || d==35,blindSpotSD,priorSD), log = FALSE)
  }
  ...
  while (!all(isFinished <- unlist(lapply(states, GEST.stop)) | 
    ...
    #step it
    r <- GEST.step(states[[j]],update_psychoSD = configuration$uSD, optimize_psychoSD = configuration$oSD,psychoFn = configuration$fn, psychoFp = configuration$fp)
    ...
    if (!identical(nFinishedLocsBeforeTrial,nFinishedLocsAfterTrial) && stopCriterionReached == 0 && length(examineSubset)!=nVisibleNodes) {
      ## 1.
