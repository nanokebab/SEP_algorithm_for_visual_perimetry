GESTwrapper.dynamicFinal <- function(configuration, locations, binSize) {
  ...
  doFinalInference <- configuration[[16]]
  ...
  for (d in 1:nVisibleNodes) {
    ...
    nvDistrs[visibleNodes[[d]],] <- dnorm(supportCRF, normThresholds[[d]], ifelse(d==26 || d==35,blindSpotSD,priorSD), log = FALSE)
  }
  invisibleNodes<-which(apply(nvDistrs,1,function(x) all(is.na(x))))
  for (d in 1:length(invisibleNodes)) {
    nvDistrs[invisibleNodes[[d]],] <- dnorm(supportCRF, invisibleNodeMean, priorSD, log = FALSE)
  }
  ...
  while (!all(isFinished <- unlist(lapply(states, GEST.stop)) | 
    ...
    if (!identical(finishedLocationsPrevious,finishedLocationsAfter) && stopCriterionReached == 0) {
      if (length(examineSubset)!=nVisibleNodes) {
        ...
        if (updatePriors=="Full") {
          ...
          # futureNodes <- which(vfCurr$node.id %in% visibleNodes[futureLocations])
          for (j in 1:length(futureLocations)) {
            # mean <- vfCurr$node.pot[futureNodes[[j]],]
        ...
        } else if (updatePriors=="Not") {
        } else
          stop("shit happend at updatePriors condition")
        ...
        if (length(futureLocations) != 1) {
          ...
          # stop adding new locations if stop criterion is reached
    ...
    # if it's the last step try to interpolate the intermediate locations
    
    # if (all(isFinished <- unlist(lapply(states, GEST.stop)) | !(seq(1,nVisibleNodes) %in% examineSubset)))
    if (doFinalInference && stopCriterionReached == 1 && length(which(!isFinished)) == 1 && !identical(finishedLocationsPrevious,finishedLocationsAfter)) {
#       print("this should be the last step now!!")
#       break
      # clamp finished locations
      clampValues <- sapply(states[finishedLocationsAfter],function(x) which.max(changeSupport(t(x$pdf),supportStates,supportCRF)))
      clampNodes <- visibleNodes[finishedLocationsAfter]
...
#     if (loop > 3 && (totThresh[[loop-2]]-totThresh[[loop]]) < -4) {
#       if (loop < 80) {
#         print("Oww, criterion was reached before 80 steps...")
#       } else {
#         print(paste("Examination stopped after", loop, "steps due to convergence"))
#         break
#       }
#     }
  }
