GESTwrapper.dynamicFinal <- function(configuration, locations) {
...
#   vf$node.pot <- nodePots
  ...
  nvDistrs <- matrix(,nrow=nNodes,ncol=length(supportCRF))
  nodePot_blindSpot <- blindSpotPrior <- dsn(supportCRF, xi=0, omega=10, alpha=5, log=FALSE)
...
#       break
    ...
    if (!identical(nFinishedLocsBeforeTrial,nFinishedLocsAfterTrial) && 
      ...
      clampValues <- sapply(states[nFinishedLocsAfterTrial],function(x) which(round(x$stairResult,0.1)==supportCRF))
      ...
      updatePriors <- configuration$updatePriors     
      if (updatePriors=="Full" | updatePriors=="Peaks") {
        if (length(futureLocations) != 1) {
        for (j in 1:length(futureLocations)) {
          states[[futureLocations[[j]]]]$currentLevel <- supportCRF[which.max(newPriorsForFutureLocations[j,])]
        }
        } else {
          states[[futureLocations]]$currentLevel <- supportCRF[which.max(newPriorsForFutureLocations)]
    ...
    if (doFinalInference == 1 && length(examineSubset)!=nVisibleNodes && all(isFinished <- unlist(lapply(states, dynamic.stop)) | !(seq(1,nVisibleNodes) %in% examineSubset))) {
      ...
      for (j in 1:length(untestedLocations)) {
        states[[untestedLocations[[j]]]]$stairResult <- supportCRF[which.max(estimateForUntestedLocations[j,])]
      }
      
    }
    
    # write down total deviation after this step
    totDev <- c(totDev, sum(abs(sapply(locations, function(x) x[[3]]) - sapply(states, function(x) x$currentLevel) )) )
    totThresh <- c(totThresh, sum(sapply(states, function(x) {tail(x$currentLevel,1)}) ))
    
  }
  
  #write down final stats
  if (finalEstimate == "mode" | finalEstimate == "median") {
  ...
  testedLocations <- which(unlist(lapply(states, dynamic.stop)))
