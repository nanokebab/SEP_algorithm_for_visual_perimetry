GESTwrapper.dynamicFinal <- function(configuration, locations, binSize=NULL) {
  ...
  while (!all(isFinished <- unlist(lapply(states, GEST.stop)) | 
    ...
    #step it
    r <- GEST.step(states[[j]],configuration = configuration) #,update_psychoSD = configuration$uSD, optimize_psychoSD = configuration$oSD,psychoFn = configuration$fn, psychoFp = configuration$fp)
