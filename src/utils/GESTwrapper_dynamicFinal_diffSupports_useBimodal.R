GESTwrapper.dynamicFinal <- function(configuration, locations, binSizeStates=0.5) {
  ...
  states <- lapply(locations, function(loc) {
      ...
      prior <- dsn(supportStates, xi=0, omega=10, alpha=5, log=FALSE)
    }
      ...
      if (length(futureLocations) != 1) {
        ...
        if (max(combinedValue[futureLocations]) < stopCombVal) {
          ...
          if (updatePriors == "Product") {
            dist2<-dnorm(supportStates, inferredThresholdNewLoc, 6, log = FALSE) + 0.03
