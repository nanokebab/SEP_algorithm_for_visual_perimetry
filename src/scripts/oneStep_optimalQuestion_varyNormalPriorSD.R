oneStep_optimalQuestion_varyNormalPriorSD <- function() {

  # Display how question to be asked changes with prior std in the different QUEST strategies and the Greedy
  ...
  for (h in 1:4) {
    par(col = myColors[h])
    plot(SDs,yVals[h,],type='l',xlab = "Prior SD", ylab = "Question [dB]", 
         xlim = c(priorSDstart-offset, xLim), ylim = c(priorMean-offset, yLim)
    )
    par(new = TRUE)
  }
  title(main="Optimal question acc. to greedy (prior normal)")
  legend("topleft", inset = .02, c("optimal solution","mean","median","mode"), fill = myColors, horiz = TRUE)
