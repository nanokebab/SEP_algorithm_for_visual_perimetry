oneLocation_Error_GreedyVaryPsychoSD <- function() { 
  
  ## produces a plot that shows how the greedy algorithm approaches the true threshold step by step. The different
  # curves represent different combinations of std used for performing the update (uSD) and performing the computation
  # for finding the 'optimal' question using the greedy (oSD)
  ...
  # DEFS
  trueThreshold <- 10   # define threshold
  ...
  stim <- c('greedy')
  configuration <- list(c(1.5,0),c(1.5,0.25),c(1.5,0.5),c(1.5,0.75),c(1.5,1),c(1.5,1.25),c(1.5,1.5),c(1.5,1.75))
