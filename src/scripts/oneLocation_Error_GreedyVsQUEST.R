oneLocation_Error_GreedyVsQUEST <- function() {   
    
    ## Show convergence of Greedy vs mean-, mode- and median-QUEST
    ...
    nrExp <- 5  # define nr of subsequent experiments
    binSize <- 0.1
    stopVal <- 50
    priorMean <- 5
    priorSD <- 3
    ...
    stim <- c('mean','median','greedy')
    configuration <- list(c(5,1))
    ...
    for (k in 1:length(stim)) {
      ...
      while (!all(st <- unlist(lapply(states, GEST.stop)))) {
        ...
        oSD <- configuration[[1]][1]
        uSD <- configuration[[1]][2]
        # step it
