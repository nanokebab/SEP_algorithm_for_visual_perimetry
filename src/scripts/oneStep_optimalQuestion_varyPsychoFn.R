oneStep_optimalQuestion_varyPsychoFn <- function() {  

  # Display how question to be asked according to Greedy changes with Fn (frac. false neg.) of psychometric function
  ...
  Fns <- seq(from=psychoFnStart,to=psychoFnEnd-(psychoFnEnd-psychoFnStart)/nrSteps,by=(psychoFnEnd-psychoFnStart)/nrSteps)
  
  for (q in 1:nrSteps) {
    stepSize <- (psychoFnEnd-psychoFnStart)/nrSteps
    psychoFn <- psychoFnStart+(q-1)*stepSize
    # compute vectors
  ...
  for (h in 1:4) {
    ...
    plot(Fns,yVals[h,]/yVals[2,],type='l',xlab = "Fn of psychometric function", ylab = "Question\n(normalized by mean)", 
         xlim = c(0, xLim), ylim = c(0.5, yLim)
    )
    par(new = TRUE)
    par(op)
  }
